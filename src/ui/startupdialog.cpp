// TODOs addressed in this revision:
// 1. Theme selector added to left panel — persisted via QSettings.
// 2. Open dialog now accepts AkisVG projects AND MP4/MKV video files.
//    When a video file is selected the action is set to ImportVideo so the
//    caller can handle it separately (MainWindow will need to wire this up).
// 3. Logo: deferred — using the painted geometric logo in paintEvent() for now.
...
void StartupDialog::setupUI()
{
    ...
    m_openBtn->setStyleSheet("QPushButton { background:transparent; color:#555; border:1px solid #222; border-radius:5px; }");
    connect(m_openBtn, &QPushButton::clicked, this, &StartupDialog::onOpenClicked);
    ll->addWidget(m_openBtn);
    root->addWidget(left);

    // ── RIGHT PANEL (Canvas Config Restored) ────────────────────────────────
    QWidget *right = new QWidget();
    right->setStyleSheet("background:#0f0f0f;");
    QVBoxLayout *rl = new QVBoxLayout(right);
    rl->setContentsMargins(36, 36, 36, 28);

    QLabel *newTitle = new QLabel("New Project");
    newTitle->setStyleSheet("color:white; font-size:20px; font-weight:bold;");
...
{
    QSettings s("AkisVG", "AkisVG");
    QStringList recents = s.value("recentProjects").toStringList();
...
void StartupDialog::onPresetChanged(int index)
{
    ...
    // "Custom" preset leaves spinboxes editable
...
int     StartupDialog::fps()          const {
    return m_fpsComboo->currentData().toInt();
}
...
void StartupDialog::paintEvent(QPaintEvent *event)
{
    ...
    // Position the logo in the top-left area (32, 32)
    painter.translate(48, 56);

    int size = 52;
    float half = size / 2.0f;

    // Define the geometric path (A Star/Compass hybrid)
    QPainterPath path;
    ...
    // Inner geometric "shards" (Matching your star reference)
    path.moveTo(half, 5);
    ...
    // Fill with a deep Crimson Gradient
    QLinearGradient grad(0, 0, size, size);
    grad.setColorAt(0, QColor("#e74c3c")); // Bright Crimson
    grad.setColorAt(1, QColor("#8e1c12")); // Deep Blood Red

    painter.fillPath(path, grad);

    // Add a sharp white highlight stroke for that "Vector" look
