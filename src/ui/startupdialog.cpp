...
{
    QSettings s("AkisVG", "AkisVG");
    // FIX #24: Use the same key as MainWindow::addToRecentFiles ("recentFiles")
    // Also merge legacy "recentProjects" key so old entries are not lost
...
{
    QSettings s("AkisVG", "AkisVG");
    // FIX #24: Use same key as MainWindow::addToRecentFiles
    QStringList recents = s.value("recentFiles").toStringList();
