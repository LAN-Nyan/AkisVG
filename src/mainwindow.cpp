...
void MainWindow::setupCanvas()
{
    ...
    m_splineOverlay->setView(m_canvasView);  // direct view ref — no parent-chain guessing
    ...
    connect(m_canvas, &VectorCanvas::contextMenuRequestedAt, this, [this](const QPoint &globalPos, const QPointF &scenePos) {
        ...
        QAction *pasteCtxAct = menu.addAction("Paste");
        menu.addSeparator();
        ...
        pasteCtxAct->setEnabled(!m_clipboard.isEmpty());
        ...
        if (chosen == groupAct) {
            ...
            m_clipboard = selected;
        } else if (chosen == pasteCtxAct) {
            if (!m_clipboard.isEmpty() && m_project) {
                Layer *layer = m_project->currentLayer();
                int frame = m_project->currentFrame();
                QList<VectorObject*> pasted;
                for (VectorObject *obj : m_clipboard) {
                    VectorObject *clone = obj->clone();
                    clone->moveBy(10, 10);
                    layer->addObjectToFrame(frame, clone);
                    pasted.append(clone);
                }
                m_canvas->refreshFrame();
                if (SelectTool *st = qobject_cast<SelectTool*>(m_toolBox->getTool(ToolType::Select))) {
                    st->setSelectedObjects(pasted);
                    m_canvas->showSelectionOverlays(pasted);
                }
            }
...
void MainWindow::createMenus()
{
    ...
    // ── Copy / Paste ──────────────────────────────────────────────────────
    QAction *copyMenuAct = m_editMenu->addAction("&Copy", this, [this]() {
        if (!m_canvas) return;
        QList<VectorObject*> sel;
        if (SelectTool *st = qobject_cast<SelectTool*>(m_toolBox->getTool(ToolType::Select)))
            sel = st->selectedObjects();
        if (!sel.isEmpty()) m_clipboard = sel;
    });
    copyMenuAct->setShortcut(QKeySequence::Copy);

    QAction *pasteAct = m_editMenu->addAction("&Paste", this, [this]() {
        if (m_clipboard.isEmpty() || !m_canvas || !m_project) return;
        Layer *layer = m_project->currentLayer();
        if (!layer) return;
        int frame = m_project->currentFrame();
        QList<VectorObject*> pasted;
        for (VectorObject *obj : m_clipboard) {
            VectorObject *clone = obj->clone();
            clone->moveBy(10, 10); // offset so paste is visually distinct
            layer->addObjectToFrame(frame, clone);
            pasted.append(clone);
        }
        m_canvas->refreshFrame();
        // Auto-select the newly pasted objects
        if (SelectTool *st = qobject_cast<SelectTool*>(m_toolBox->getTool(ToolType::Select))) {
            st->setSelectedObjects(pasted);
            m_canvas->showSelectionOverlays(pasted);
        }
...
void MainWindow::onLassoCut(const QPolygonF &poly)
{
    ...
    for (VectorObject *display : displayItems) {
        ...
        // Skip items that are children inside a group — the ObjectGroup entry
        // in displayItems represents the whole group and is handled below.
        if (src->parentItem() != nullptr) continue;
...
void MainWindow::onLassoPull(const QPolygonF &poly, QPointF /*dragStart*/)
{
    ...
    for (VectorObject *display : displayItems) {
        ...
        // Skip children nested inside a group
        if (src->parentItem() != nullptr) continue;

        // ObjectGroup — pull the whole group as one unit
        if (dynamic_cast<ObjectGroup*>(src)) {
            pulledPieces << src;
            anyChange = true;
            continue;
