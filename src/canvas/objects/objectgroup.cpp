...
ObjectGroup::ObjectGroup(const QString &name, QGraphicsItem *parent)
    ...
    , m_groupName(name)
{
    // Groups are selectable but NOT independently movable — SelectTool drives
    // movement via moveBy() on the source object. Leaving ItemIsMovable true
    // causes Qt's scene drag machinery to double-apply the displacement and
    // snap back to the old position when refreshFrame() rebuilds clones.
