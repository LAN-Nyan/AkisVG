...
void PathObject::rebuildSmoothPath()
{
    ...
    // Pad so each interior segment i always has valid p0 = i-1 and p3 = i+2
    QList<QPointF> pts;
    pts << m_rawPoints.first();
    for (const QPointF &pt : m_rawPoints) pts << pt;
    pts << m_rawPoints.last();
