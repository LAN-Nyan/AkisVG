...
{
    if (!m_view) return scenePos;
    return QPointF(m_view->mapFromScene(scenePos));
}

QPointF SplineOverlay::viewportToScene(const QPointF &vp) const
{
    if (!m_view) return vp;
    return m_view->mapToScene(vp.toPoint());
}
...
void SplineOverlay::paintEvent(QPaintEvent *)
{
    ...
    QRectF cvr = canvasOverlayRect();
    if (cvr.isEmpty()) cvr = QRectF(rect());
